            ...
            return {
              ...
              positions: info.lbPairPositionsData.map(position => {
                ...
                return {
                  ...
                  balanceX: toUiAmount(new BN(position.positionData.totalXAmount.split('.')[0]), baseDecimals),
                  balanceY: toUiAmount(new BN(position.positionData.totalYAmount.split('.')[0]), quoteDecimals),
                  unclaimedFeeX: toUiAmount(position.positionData.feeX, baseDecimals),
                  unclaimedFeeY: toUiAmount(position.positionData.feeY, quoteDecimals),
                  totalClaimedFeeX: toUiAmount(position.positionData.totalClaimedFeeXAmount, baseDecimals),
                  totalClaimedFeeY: toUiAmount(position.positionData.totalClaimedFeeYAmount, quoteDecimals),
