...
toolkit.action(
  {
    action: "claimAndCloseMultiplePositions",
    actionDescription: "Claim any accumulated trading fees and protocol rewards from multiple user positions in Meteora DLMM liquidity pools, and close all specified positions in a single transaction.",
    payloadDescription: {
      "positions": {
        "type": "array",
        "description": "An array of position objects to claim and close. Each object should contain lbPair and position addresses.",
        "required": true,
        "items": {
          "type": "object",
          "properties": {
      ...
      "claimRewards": {
        "type": "boolean",
        "description": "Whether to claim accumulated fees and rewards before closing positions. Defaults to true.",
        "required": false,
        "default": true
      }
    }
  ...
  async (ctx: ActionContext, payload: any = {}) => {
    try {
      ...
      const result = await txApi.createTransaction(
        "meteora/dlmm/claim-and-close-multiple-positions",
        ctx,
        {
          positions: payload.positions,
          claimRewards: payload.claimRewards !== false // Default to true
        }
      );
      
      return ctx.result({
        ...result,
        message: `Successfully processed ${payload.positions.length} positions for claim and close`,
        processedPositions: payload.positions.length
      });
    } catch (error) {
      return ctx.result({ error: `Failed to create batch transaction: ${error}` });
    }
  }
);